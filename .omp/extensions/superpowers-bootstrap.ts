...
OMP provides native \`task\` and \`todo\` tools. Use \`task\` for Superpowers subagent workflows — the \`agent\` field selects the subagent type and implicitly the model tier (\`sonic\` = fast/small, \`task\` = default, \`reviewer\`/\`plan\` = slow/most capable). Use \`todo\` for task tracking. Treat older Superpowers references to \`Task\`/subagents and \`TodoWrite\`/task lists as the matching OMP \`task\` and \`todo\` tools.`;
}
