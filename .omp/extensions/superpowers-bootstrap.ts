...
OMP provides native \`task\` and \`todo\` tools. Use \`task\` for Superpowers subagent workflows — the \`agent\` field selects the subagent type and implicitly the model tier: \`sonic\` = pi/smol (cheap/fast), \`task\` = pi/task (standard/default), \`reviewer\`/\`plan\` = pi/slow (most capable). When the SDD skill says to use a "cheap model" for a mechanical task, dispatch with \`agent: "sonic"\`; when it says "standard model", use \`agent: "task"\`; when it says "most capable", use \`agent: "reviewer"\` or \`agent: "plan"\`. Always specify \`agent\` — omitting it silently defaults to \`pi/task\`, defeating SDD's Model Selection. Use \`todo\` for task tracking. Treat older Superpowers references to \`Task\`/subagents and \`TodoWrite\`/task lists as the matching OMP \`task\` and \`todo\` tools.`;
}
