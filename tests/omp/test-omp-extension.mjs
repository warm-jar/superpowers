...
const extensionPath = resolve(repoRoot, '.omp/extensions/superpowers-bootstrap.ts');
const ompToolsPath = resolve(repoRoot, 'skills/using-superpowers/references/omp-tools.md');
...
test('package.json declares an omp package with skills and extension resources', async () => {
  const pkg = await readPackageJson();

  assert.equal(pkg.name, 'superpowers');
  assert.ok(pkg.pi?.extensions, 'pi.extensions should still be declared for pi compatibility');
  assert.deepEqual(pkg.omp.extensions, ['./.omp/extensions/superpowers-bootstrap.ts']);
  assert.deepEqual(pkg.omp.skills, ['./skills']);
});

test('extension registers lifecycle hooks (no resources_discover — omp auto-discovers skills)', async () => {
  ...
  // OMP's omp-plugins provider auto-discovers skills/ from the package — no
  // resources_discover handler needed (unlike pi).
  assert.equal((handlers.get('resources_discover') ?? []).length, 0, 'omp extension should not register resources_discover');
...
test('omp tools reference documents omp-specific mappings', async () => {
  assert.equal(existsSync(ompToolsPath), true, 'omp-tools.md should exist');
  const text = await readFile(ompToolsPath, 'utf8');

  for (const expected of ['task', 'todo', 'read', 'write', 'edit', 'bash', 'grep', 'glob', 'Skill']) {
